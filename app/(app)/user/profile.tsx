...
import { LogOut, ScanQrCode, UserRound } from "lucide-react-native";
        ...
        <Box className="bg-white mt-3 py-5 px-3">
          <Pressable
            onPress={async () => {
                ...
                return router.navigate("/(app)/user/qr-code")}}
            className="flex-row items-center gap-2"
          >
            <Box className="p-2.5 bg-[#f1f2f4] rounded-full relative -left-1">
              <ScanQrCode size={18} strokeWidth={1} />
            </Box>
            <Box>
              <Heading className="font-medium text-sm">QR Code</Heading>
              <Text className="text-sm text-typography-400">Scannable qr code for friends</Text>
            </Box>
          </Pressable>
        </Box>
